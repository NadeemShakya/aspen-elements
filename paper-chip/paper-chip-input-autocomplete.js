...
class PaperChipInputAutocomplete extends PolymerElement {
    ...
    return html`
      <style>
        ...
        paper-item {
          ...
          --paper-item-min-height: var(
            --paper-chip-autocomplete-item-height,
            48px
          );
          --paper-item: {
        ...
        paper-listbox {
          --paper-listbox-background-color: var(
            --paper-chip-autocomplete-suggestion-background,
            var(--primary-background-color)
          );
          --paper-listbox-color: var(
            --paper-chip-autocomplete-suggestion-text-color,
        ...
        paper-input {
          --paper-input-container-focus-color: var(
            --paper-chip-autocomplete-focus-color,
            var(--primary-color)
          );
          ...
          --paper-input-container-label: {
            ...
            -webkit-font-smoothing: antialiased;
            font-size: var(
              --paper-chip-input-autocomplete-label-font-size,
              13px
            );
          }
        }
      ...
      <iron-a11y-keys
        target="[[_paperInputElement]]"
        keys="enter"
        on-keys-pressed="_onKeyEnter"
      ></iron-a11y-keys>
      <iron-a11y-keys
        target="[[_paperInputElement]]"
        keys="backspace"
        on-keys-pressed="_onKeyBackspace"
      ></iron-a11y-keys>
      <iron-a11y-keys
        target="[[_paperInputElement]]"
        keys="esc"
        on-keys-pressed="_onKeyEsc"
      ></iron-a11y-keys>

      <iron-a11y-keys
        target="[[_paperInputElement]]"
        keys="down"
        on-keys-pressed="_onKeyUpOrDown"
      ></iron-a11y-keys>
      <iron-a11y-keys
        target="[[_paperInputElement]]"
        keys="up"
          ...
          <div id="slot2" slot="prefix">
            <dom-repeat items="[[items]]">
              <template>
                [[item.name]]
                <paper-chip
                  on-tap="_onIndividualTagClicked"
                  id="paper-chip-[[item]]-[[index]]"
                  label="[[item]]"
                  closable$="[[closable]]"
          ...
          <paper-material class$="[[_autocompleteClass]]" elevation="1">
            <paper-listbox id="listbox">
              <dom-repeat items="[[_filteredSource]]">
                <template>
                  <paper-item
                    id="paperItemAutocomplete"
                    on-tap="_selectPaperItem"
                    closable$="[[closable]]"
                    >[[item.text]]
  ...
  _filterItems() {
    ...
    if (filteredByString.length > 0 && this.items.length) {
      this._filteredSource = filteredByString.filter((element) =>
        this._filterByExistingPaperChip(element)
      );
    } else {
  ...
  _onIndividualTagClicked(e) {
    const tag = e.model.__data.item;
    this.dispatchEvent(
...
window.customElements.define(
  PaperChipInputAutocomplete.is,
  PaperChipInputAutocomplete
);
